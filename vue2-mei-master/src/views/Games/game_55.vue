...
<div>
  ...
  <dailog-q :type_code=type_code :game_code=51 :money="money_s" :round="round" :lotteryM="objects" v-if="showDailogQ" v-on:listenToChildEvent="showMsgFromChild"></dailog-q>
    ...
    if (oldTime && newTime <= oldTime) {
      ...
      this.$http.post(getTime(), JSON.stringify(params.params)).then(res => {
        ...
        if (res.data.msg == "4001") {
          ...
          // localStorage.setItem('bj_time', res.data.next.endtime)
